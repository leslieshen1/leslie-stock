  ...
  return (
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <header className="mb-10">
        <p className="mb-2 text-xs font-medium uppercase tracking-wider text-violet-600">Guide · 2026.06 更新</p>
        <h1 className="text-3xl font-semibold tracking-tight text-zinc-900">如何买美股</h1>
      ...
      <section className="mb-10 rounded-2xl border border-zinc-200 bg-white p-6">
        <h2 className="mb-4 text-base font-semibold text-zinc-800">为什么不推荐传统券商</h2>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[560px] text-sm">
            <thead>
              <tr className="border-b border-zinc-200 text-left text-xs text-zinc-500">
                <th className="py-2 pr-4"></th>
                <th className="py-2 pr-4">传统券商<br /><span className="font-normal text-zinc-400">盈透 / 嘉信 / 富途</span></th>
                <th className="py-2 text-violet-700">交易所代币化股票<br /><span className="font-normal text-violet-400">Binance / Bitget</span></th>
              </tr>
            </thead>
            <tbody className="text-zinc-700">
              {[
                ...
                <tr key={i} className="border-b border-zinc-100">
                  <td className="py-2.5 pr-4 font-medium text-zinc-500">{k}</td>
                  <td className="py-2.5 pr-4 text-zinc-600">{a}</td>
                  <td className="py-2.5 font-medium text-zinc-800">{b}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* 两条路径 */}
      <h2 className="mb-4 text-xl font-semibold text-zinc-900">两条主流路径</h2>
      <div className="mb-10 grid grid-cols-1 gap-5 md:grid-cols-2">
        {/* Binance */}
        <section className="rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50/40 to-white p-6">
          <div className="mb-3 flex items-center gap-2">
            <span className="text-xl">🟡</span>
            <h3 className="text-lg font-semibold text-zinc-900">Binance</h3>
            <span className="rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-700">2026.06 新上线</span>
          </div>
          ...
          <ol className="space-y-2 text-sm text-zinc-700">
            {[
              ...
              <li key={i} className="flex gap-2">
                <span className="shrink-0 font-mono text-xs text-amber-600">{i + 1}.</span>
                <span>{s}</span>
              </li>
            ))}
          </ol>
        ...
        {/* Bitget */}
        <section className="rounded-2xl border border-cyan-200 bg-gradient-to-br from-cyan-50/40 to-white p-6">
          <div className="mb-3 flex items-center gap-2">
            <span className="text-xl">🔵</span>
            <h3 className="text-lg font-semibold text-zinc-900">Bitget</h3>
            <span className="rounded bg-cyan-100 px-1.5 py-0.5 text-[10px] font-medium text-cyan-700">已上线 · 自托管</span>
          </div>
          <p className="mb-4 text-sm leading-relaxed text-zinc-600">
          ...
          <ol className="space-y-2 text-sm text-zinc-700">
            {[
              ...
              <li key={i} className="flex gap-2">
                <span className="shrink-0 font-mono text-xs text-cyan-600">{i + 1}.</span>
                <span>{s}</span>
              </li>
            ))}
          </ol>
      ...
      {/* RWA 闭环 */}
      <section className="mb-10 rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50/50 to-white p-6">
        <h2 className="mb-2 text-base font-semibold text-violet-800">💡 这其实就是「链上美元买链上股票」</h2>
      ...
      {/* 风险 */}
      <section className="mb-10 rounded-2xl border border-rose-200 bg-rose-50/50 p-6">
        <h2 className="mb-3 text-base font-semibold text-rose-800">⚠️ 必读风险（别跳过）</h2>
      ...
      <p className="text-center text-xs leading-relaxed text-zinc-400">
        ...
        <br />
        <Link href="/whales" className="text-violet-500 hover:underline">看名人在买什么 →</Link>
      </p>
    </main>
  );
}
